...
import { ScrollbarGeometry, ScrollbarPosition } from '@worthbank/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[worthbankScrollbar]',
    exportAs: 'worthbankScrollbar'
})
export class WorthBankScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_worthbankScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() worthbankScrollbar: boolean = true;
    @Input() worthbankScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'worthbankScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.worthbankScrollbar = coerceBooleanProperty(changes.worthbankScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.worthbankScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'worthbankScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.worthbankScrollbarOptions.currentValue);
    ...
    isEnabled(): boolean
    {
        return this.worthbankScrollbar;
    }
